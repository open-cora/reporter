"""The two calls this reporter makes to the keeper, and what it does with a no.
...
address. The keeper keys its cache on `(principal_id, key, surface_id)`, so a
...
def dataset_key_for(external_ref_value: str) -> str:
    ...
    Derived rather than remembered, which is the whole point. The keeper's store
...
class RequestRefusedError(Exception):
    ...
    whatever is driving this, and only it can decide:
        ...
        404  the execution or the step is not there, which means the
             reference in the engine's metadata and the keeper disagree.
        409  the report does not follow the engine state the keeper holds. a
...
class KeeperClient:
    """Sends what the translator produced, to the keeper a config names."""
    ...
    def register_dataset(self, intent: RegisterDataset, *, scheme: str) -> UUID:
        ...
        travels as `None` when the store holds no ending yet, and the keeper
